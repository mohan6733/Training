trigger PositionSharingTrigger on Position__c (after insert, after update) {
		...
		if (Trigger.isUpdate){
			...
			SalarySharing.deleteHiringMgrSharing(posIdToNewMgrIdMap);
		JobAppSharing.deleteHiringMgrSharing(posIdToNewMgrIdMap);
			// 2. Remove Old salary VP sharing
		SalarySharing.deleteVPSharing(posIdToDeptMap);
		}
		// 3. Add Hiring Mgr to Read/Write sharing
		PositionSharing.addSharing(nonApprovedPositions,'Edit');
	 	JobAppSharing.createSharing(Trigger.new,'Hiring_Manager__c','Edit');
		...
		PositionSharing.addSharing(approvedPositions,'Read'); 
		PositionSharing.closeHiringMgrSharing(approvedPositions);	
		JobAppSharing.createSharing(closedPositions,'Hiring_Manager__c','Read');
		JobAppsharing.closeHiringMgrSharing(closedPositions);
		// 6. Add Entire Organization sharing IFF Approved
		PositionSharing.addSharing(approvedPositions,'Approved_Position__c','Read');
